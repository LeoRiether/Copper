...
Direction Direction::fromInput() {
  ...
  self.x = input.IsKeyDown(MOVE_LEFT_KEY)
               ? Left
               : input.IsKeyDown(MOVE_RIGHT_KEY) ? Right : NoneX;

  self.y = input.IsKeyDown(MOVE_UP_KEY)
               ? Up
               : input.IsKeyDown(MOVE_DOWN_KEY) ? Down : NoneY;
  return self;
}
