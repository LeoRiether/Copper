...
Game::Game(const char *title, int width, int height) {
  ...
  if (IMG_Init(img_flags) != img_flags)
    sdlfail("couldn't initialize images");

  int mix_flags = MIX_INIT_MP3 | MIX_INIT_OGG | MIX_INIT_OPUS;
  if (Mix_Init(mix_flags) != mix_flags)
    sdlfail("couldn't initialize audio");
  ...
  if (TTF_Init() != 0)
    fail2("TTF_Init failed. Cause: %s", TTF_GetError());

  window = SDL_CreateWindow(title, SDL_WINDOWPOS_CENTERED,
                            SDL_WINDOWPOS_CENTERED, width, height, 0);
  if (!window)
    sdlfail("couldn't create window");

  renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_PRESENTVSYNC);
  if (!renderer)
    sdlfail("couldn't create renderer");
...
void Game::Run() {
  ...
  while (!stateStack.empty() && !stateStack.back()->QuitRequested()) {
    ...
    int32_t deltaMs = (int32_t)SDL_GetTicks() - frameStart;
