...
Text::Text(GameObject &associated, string fontFile, int fontSize,
    ...
    : Component(associated), font(nullptr), texture(nullptr),
      text(std::move(text)), style(style), fontFile(std::move(fontFile)),
      fontSize(fontSize), color(color), wrapWidth(wp) {
  if (this->text.empty()) {
    warn("constructor called with empty text. This will probably not work "
...
void Text::SetText(string t) {
  if (t.empty()) {
    warn("SetText called with empty text. This will probably not work "
         "because the texture will be empty as well");
  }
  text = std::move(t);
  RemakeTexture();
...
void Text::RemakeTexture() {
  ...
  switch (style) {
  case Text::Solid: {
    surface = TTF_RenderUTF8_Solid_Wrapped(font->inner, text.c_str(), color,
                                           wrapWidth);
    break;
  }
  case Text::Shaded: {
    surface = TTF_RenderUTF8_Shaded_Wrapped(font->inner, text.c_str(), color,
                                            SDL_Color{0, 0, 0, 0}, wrapWidth);
    break;
  }
  case Text::Blended: {
    surface = TTF_RenderUTF8_Blended_Wrapped(font->inner, text.c_str(), color,
                                             wrapWidth);
    break;
  }
  }
